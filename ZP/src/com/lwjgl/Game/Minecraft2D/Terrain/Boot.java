...
public class Boot {
	...
	private Terrain selection = Terrain.STONE;
	...
	private void drawSelectionBox() {
		...
		if (grid.getAt(selector_x, selector_y).getType() != Terrain.AIR
				|| selection == Terrain.AIR) {
			...
			glColor4f(1f, 1f, 1f, 0.5f);
			new TerrainSet(selection, selector_x * World.BLOCK_SIZE, selector_y
	...
	private void input() {
		...
		while (Keyboard.next()) {
			...
			if (Keyboard.getEventKey() == Keyboard.KEY_1) {
				selection = Terrain.STONE;
			}
			if (Keyboard.getEventKey() == Keyboard.KEY_2) {
				selection = Terrain.DIRT;
			}
